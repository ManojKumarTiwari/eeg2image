"""
generate_vi.py — EEG Visual Imagery -> Text -> Image pipeline.

For each test EEG sample:
  1. EEG signal -> CSBrain -> TinyLlama -> predicted text
  2. Predicted label -> aMUSEd-512 text-to-image -> generated image
  3. Save side-by-side comparison: [original stimulus | generated image]

Usage:
    python generate_vi.py --num_samples 20
    python generate_vi.py --num_samples 20 --output_dir outputs/vi_images
"""
...
# Maps class index -> stimulus image filename
VI_STIMULUS_FILES = {
    0: 'Animal_dog.jpg',
    1: 'Animal_bird.jpg',
    2: 'Animal_fish.jpg',
...
# Prompts tuned for aMUSEd-512 (short, direct descriptions work best)
VI_PROMPTS = {
...
def make_comparison_image(
    ...
    """Create a side-by-side comparison: [stimulus | generated] with labels."""
...
def main():
    ...
    parser.add_argument('--output_dir',      type=str, default='outputs/vi_images')
    ...
    # EEGLanguageModel required params
    ...
    print("=" * 60)
    print("Stage 1: Loading EEG -> Text model")
    print("=" * 60)
    ...
    # Free EEG-LLM memory before loading image model
    del model
    torch.cuda.empty_cache()
    ...
    prompts = [
        VI_PROMPTS.get(r['pred_label'], VI_CLASS_NAMES[r['pred_label']] if r['pred_label'] >= 0 else "abstract shape")
        for r in results
    ]
    ...
    for i, (r, gen_img) in enumerate(zip(results, images)):
        ...
        gen_path = os.path.join(args.output_dir, f'generated_{i:04d}_true{true_label}_pred{pred_label}.png')
        ...
        comp_path = os.path.join(args.output_dir, f'comparison_{i:04d}_true{true_label}_pred{pred_label}.png')
        comp.save(comp_path)
        comparison_imgs.append(comp)

        status = 'CORRECT' if pred_label == true_label else 'WRONG'
        print(f"  [{i:03d}] true={VI_CLASS_NAMES[true_label]:10s} pred={VI_CLASS_NAMES[pred_label] if pred_label>=0 else 'unknown':10s} {status}")
