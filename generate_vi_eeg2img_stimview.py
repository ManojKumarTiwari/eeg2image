"""
generate_vi_eeg2img_stimview.py — Pipeline 3 (EEG->CLIP->Image) for stimulus-view experiment.

Same as generate_vi_eeg2img.py but defaults to the stimview LMDB and mapper
checkpoint trained on the 4s image-shown window (rather than the 4s imagery window).

Usage:
    python generate_vi_eeg2img_stimview.py --num_samples 20
    python generate_vi_eeg2img_stimview.py \\
        --mapper_path pth_downtasks/eeg_direct_stimview/mapper_epochN.pth \\
        --num_samples 20 --output_dir outputs/vi_eeg2img_stimview
"""
...
def make_comparison_image(stimulus_path, gen_img, true_label, pred_label, idx, size=512):
    ...
    draw.text((10, size + 8), f"[{idx}] EEG->class->SD1.5  [stimview]", fill=(60, 60, 60))
    return canvas
...
def main():
    ...
    parser.add_argument('--mapper_path',   type=str,
                        default='pth_downtasks/eeg_direct_stimview/mapper_epoch17.pth')
    parser.add_argument('--output_dir',    type=str,
                        default='outputs/vi_eeg2img_stimview')
    # ── Shared args (same as generate_vi_eeg2img.py) ─────────────────────────
    ...
    print("=" * 60)
    print("Stage 1: EEG -> EEGCLIPMapper classifier  [stimview experiment]")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("Stage 2: Generating images with SD 1.5 (predicted class prompts)")
    print("=" * 60)
